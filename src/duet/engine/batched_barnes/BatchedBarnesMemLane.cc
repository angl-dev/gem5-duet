#include "duet/engine/batched_barnes/BatchedBarnesMemLane.hh"
...
BatchedBarnesMemLane::BatchedBarnesMemLane(const DuetSimpleLaneParams& p)
    : DuetSimpleLane(p), _next_caller_roundrobin(0) {}

DuetFunctor* BatchedBarnesMemLane::new_functor() {
  for (DuetFunctor::caller_id_t i = 0; i < engine->get_num_callers(); ++i) {
    ...
    if (!chan.empty()) {
      auto f = new BatchedBarnesMemFunctor(this, id.id);

#include "duet/engine/barnes_gravsub_quad/DuetBarnesQuadEngine.hh"

namespace gem5 {
namespace duet {

DuetEngine::softreg_id_t DuetBarnesQuadEngine::get_num_softregs () const {
    return num_softreg_per_caller * (get_num_callers () + 1);
}

DuetFunctor::caller_id_t DuetBarnesQuadEngine::get_num_memory_chans () const {
    return 1;
}

DuetFunctor::caller_id_t DuetBarnesQuadEngine::get_num_interlane_chans () const {
    return 3;
}
...
bool DuetBarnesQuadEngine::handle_softreg_read (
        DuetEngine::softreg_id_t    softreg_id
        , uint64_t                & value
        )
{
...
void DuetBarnesQuadEngine::try_send_mem_req_all () {
    try_send_mem_req_one ( 0 );
}

void DuetBarnesQuadEngine::init () {
    DuetEngine::init ();
